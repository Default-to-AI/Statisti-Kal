...
- **Accent Cobalt (#6366F1)**: NEUTRAL & INTERACTIVE — Z-scores, standard normal, interactive states, primary interactive elements (Indigo hue)
...
- `info` = `#6366F1` (indigo for neutral info and primary interactions)
...
- Closed: surface-raised bg, lg radius, structural border
- Open: surface-raised bg
- Summary: heading-section, flex, chevron icon (rotates on open), primary text, brass badges/numbers
- Content: body-base, primary text, space-y layout

### Formula Blocks
Two variants for "general formula" vs "calculated substitution":
| Variant | Accent Color | Label |
|---------|--------------|-------|
| `formula-block` | Brass | "תבנית כללית" (General Template) |
| `calc-block` | Cobalt (Indigo) | "יישום" (Application) |

Both: mono-lg, ltr direction, overflow-x-auto, lg radius, structural borders with thick colored left border, surface-raised bg
